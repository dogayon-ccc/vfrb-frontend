...
// ── Full scene ─────────────────────────────────────────────────────────────────
export default function DesignStudio3D({ cfg = {}, overlayDataUrl = null }) {
  ...
  return (
    <Canvas
      ...
      {/* Teal accent rim light — brand color */}
      <pointLight position={[-2, 1, 3]} intensity={0.7} color="#02C39A"/>
      <pointLight position={[2, -1, 2]} intensity={0.4} color="#028090"/>

      {/* ── Local lights replace Environment preset (no CDN fetch) ── */}
      {/* FF-1 FIX: Environment preset="studio" fetched HDR from DigitalOcean CDN
          which crashes WebGL in offline/Laragon environment.
          hemisphereLight provides sky+ground fill equivalent to studio preset. */}
      <hemisphereLight skyColor="#e0f7fa" groundColor="#b2dfdb" intensity={0.9}/>

      {/* ── Garment with gentle float ── */}
