...
export default function Login() {
  ...
  // Real Google Sign-In (Aug 23 2026): the backend redirects back here with
  // ?google_error=... when it rejects the attempt — most importantly, when
  // a staff/manager email is used against this customer-only flow (the
  // hard rule). Surface it in the same error banner normal login errors
  // use, then clear it from the URL so a refresh doesn't re-show it.
  useEffect(() => {
      ...
      <div style={{ minHeight:'100vh', display:'flex', fontFamily:"ui-sans-serif,system-ui,-apple-system,'Segoe UI',sans-serif", color:'#fff', background:'#06101a' }}>
        ...
        <motion.div className="lr" initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:.5 }}>
          <div style={{ width:'100%', maxWidth:400 }}>
            ...
            <button onClick={() => {
              // FIX (Aug 23 2026): this used to be a relative path
              // ('/auth/google/redirect'), which navigates within the
              // FRONTEND's own domain — a route that doesn't exist there.
              // It never reached the backend at all, which is a real part
              // of why this button was fully decorative (confirmed earlier
              // this session). Must be the backend's absolute URL.
